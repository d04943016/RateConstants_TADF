...
# Import the rate constants calculator module
basepath = os.path.dirname(os.path.abspath(__file__))
if basepath not in sys.path:
    sys.path.append(basepath)
from src import rate_constants_calculator as rcc

app = FastAPI(title="TADF Rate Constants Calculator API", 
...
def calculate_determined_rate_constants(input_data: DeterminedRateConstantsInput):
    """Calculate the rate constants that can be determined directly from experimental data"""
    try:
...
def run_script(input_data: ScriptInput):
    """Run the rate constants calculator script with the provided parameters"""
    try:
