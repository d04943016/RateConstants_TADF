...
# Add project root to path so RateConstantCalculator can be imported
...
def download():
    try:
        ...
        kPF, kDF = RCC.tau2k([tau_PF, tau_DF])
        PLQY = Phi_PF + Phi_DF
        ...
        (ks_Array, ksr_Array, ksnr_Array, kisc_Array,
         kt_Array, ktr_Array, ktnr_Array, krisc_Array) = \
            RCC.IntrinsicRateConstants(kPF, kDF, Phi_PF, Phi_DF,
                                       phi_Tnr_PL=phi_Tnr_PL_Array)

        phi_sr_Array, phi_snr_Array, phi_isc_Array = \
            RCC.phi_sr_snr_isc(ksr_Array, ksnr_Array, kisc_Array)
        phi_tr_Array, phi_tnr_Array, phi_risc_Array = \
            RCC.phi_tr_tnr_risc(ktr_Array, ktnr_Array, krisc_Array)
...
def _parse_inputs(data):
    ...
    if mode_lf_k == 0:  # lifetime
        tau_PF = float(data['pf_value']) * 1e-9   # ns -> s
        tau_DF = float(data['df_value']) * 1e-6   # us -> s
    else:               # rate constant
        kPF = float(data['pf_value']) * 1e8
        kDF = float(data['df_value']) * 1e5
        taus = RCC.k2tau([kPF, kDF])
    ...
    if mode_qy_b == 0:  # quantum yield
        ...
        Phi_PF, Phi_DF = RCC.phi_PF_DF(PLQY, tau_PF, tau_DF, B_PF, B_DF)
